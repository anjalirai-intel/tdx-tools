...
def multi_vm_instances(vm_factory, vm_ssh_pubkey):
    ...
    for index in range(_MAX_TD_GUEST):
        ...
        vm_inst = vm_factory.new_vm(VM_TYPE_TD, vmspec)
...
def worker(instance, vm_ssh_key, index, results):
    ...
    cmd = f"stress-ng --memcpy 1 --memrate 1 --memthrash 1 --vm 1 -t {_STRESS_DURATION}"
...
def test_multi_tdvm_stress_ng_mem(multi_vm_instances, vm_ssh_key):
    """
    Test TD with several memory stressors running
    Step 1: Create multiple TD guests
    Step 2: run workload stress-ng mem
