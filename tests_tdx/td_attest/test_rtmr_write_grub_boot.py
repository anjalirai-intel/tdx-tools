"""
This file includes multiple tests for RTMR write in grub boot mode
...
Implemented:
    ...
    - Case 4: Test extend RTMR with invalid privilege
      (register index 0/1 that cannot be extended in userspace)
...
from td_attest_helper import install_packages_for_td_rtmr,\
    parse_td_report_bin_files, remote_get_td_report, extend_and_check_rtmr,\
    ssh_and_extend_rtmr, shutdown_and_destroy_td, check_rtmr_extend_result
...
def base_td_guest_inst(vm_factory, vm_ssh_pubkey, vm_ssh_key):
    ...
    """
    base_td_inst = _create_td_inst(vm_factory, vm_ssh_pubkey)
    _start_td_inst(base_td_inst, vm_ssh_key)

    yield base_td_inst


def _create_td_inst(vm_factory, vm_ssh_pubkey):
    td_inst = vm_factory.new_vm(VM_TYPE_TD, boot=BOOT_TYPE_GRUB, vm_class=VMMQemu)
...
def _start_td_inst(td_inst, vm_ssh_key):
    """
    Start a td guest instance in grub boot mode
    """
    td_inst.create()
    td_inst.start()
    td_inst.wait_for_ssh_ready(timeout=1800)
...
def test_rtmr_write_with_invalid_privilege(base_td_guest_inst, vm_ssh_key, output):
    ...
    output_file = f"/tmp/tdx_rtmr_extend_result_check_{DATE_SUFFIX}.log"
    result = extend_and_check_rtmr(
             base_td_guest_inst, vm_ssh_key, digest, int(RTMR_REGISTER_WITH_INVALID_PRIVILEGE),
             output_file, output)
...
def test_rtmr_write_on_multiple_td(vm_factory, vm_ssh_pubkey, base_td_guest_inst, vm_ssh_key, output):
    ...
    """
    LOG.info("Write to rtmr in multiple TDs with same extend data and \
            check if the extend result are equal")
    digest = RTMR_TEST_EXTEND_VALUE * VALID_DIGEST_LENGTH

    # create td for comparison
    compared_td_inst = _create_td_inst(vm_factory, vm_ssh_pubkey)
    _start_td_inst(compared_td_inst, vm_ssh_key)
    ...
    ssh_and_extend_rtmr(
            base_td_guest_inst, vm_ssh_key, digest, int(DEFAULT_RTMR_REGISTER), output_file1)
    ssh_and_extend_rtmr(
            compared_td_inst, vm_ssh_key, digest, int(DEFAULT_RTMR_REGISTER), output_file2)
    ...
    # verify if the two td reports are identical
