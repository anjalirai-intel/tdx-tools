...
def test_tdx_enabled_in_bios():
    ...
    According to
    https://wiki.ith.intel.com/pages/viewpage.action?spaceKey=DCGBKC&title=TDX+BKC+recipe
...
def test_sgx_supported_in_cpuinfo():
    ...
    Even if SGX is supported by platform, user still can disable sgx by adding 'nosgx' in kernel
    command line, If sgx is appear in the /proc/cpuinfo, it really means that sgx is enabled
...
@pytest.mark.skip(reason="From kernel 5.19, tdx flag is removed by design")
...
def test_check_mktme_keys():
    """
    check number of Keys of MK-TME
    https://software.intel.com/sites/default/files/managed/a5/16/Multi-Key-Total-Memory-Encryption-Spec.pdf
...
def test_check_sgx_dev_node_exist():
    """
    check whether '/dev/sgx_provision' is exist

    Test Steps
    ----------
    Check file /dev/sgx_provision exist
    """
...
def test_check_sgx_registration_status():
    ...
    Test Steps
    ----------
    Check file /sys/firmware/efi/efivars/SgxRegistrationStatus-* exist
    """
